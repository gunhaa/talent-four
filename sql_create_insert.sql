...
CREATE TABLE "SETTLE" (
   ...
   "SETTLE_STATUS"   CHAR(1)      NOT NULL,
   "COMMISSION"   NUMBER   DEFAULT 20   NOT NULL,
   "SETTLE_DATE"   DATE   DEFAULT SYSDATE   NOT NULL,
    FOREIGN KEY (MEMBER_NO) REFERENCES TUTOR (MEMBER_NO) ON DELETE CASCADE,
    FOREIGN KEY (MEMBER_NO2) REFERENCES MEMBER (MEMBER_NO) ON DELETE CASCADE,
    FOREIGN KEY (CLASS_NO) REFERENCES CLASS (CLASS_NO) ON DELETE CASCADE
);

COMMENT ON COLUMN "SETTLE"."MEMBER_NO" IS '튜터';
COMMENT ON COLUMN "SETTLE"."MEMBER_NO2" IS '수강생번호';
COMMENT ON COLUMN "SETTLE"."CLASS_NO" IS '클래스번호';
COMMENT ON COLUMN "SETTLE"."COMMISSION" IS '결제 수수료율';
COMMENT ON COLUMN "SETTLE"."SETTLE_DATE" IS '정산신청날자';
COMMENT ON COLUMN "SETTLE"."SETTLE_STATUS" IS '정산 가능(1) 정산 완료(2)';
...
COMMENT ON COLUMN "STUDYBOARD"."STUDY_LOCATION" IS '모임장소(광대한 범위)';
...
BEGIN
    FOR I IN 1..15 LOOP -- 카테고리 개수
        randomClassNo := TRUNC(DBMS_RANDOM.VALUE(10, 25));
...
-- 입력 데이터 관련 맴버 (35명) 대입
BEGIN
    FOR I IN 501..535 LOOP
    
    INSERT INTO MEMBER
    VALUES(SEQ_MEMBER_NO.NEXTVAL,
           'user'||SEQ_MEMBER_NO.CURRVAL||'@naver.com',
           '고길동'||SEQ_MEMBER_NO.CURRVAL,
           'a'||SEQ_MEMBER_NO.CURRVAL,
           DEFAULT, DEFAULT, DEFAULT, DEFAULT);
    END LOOP;
END;
/
...
BEGIN
    FOR I IN 1..200 LOOP
        ...
        INSERT INTO STUDYBOARD (
            BOARD_NO, STUDY_NM, STUDY_START, STUDY_END, STUDY_TYPE, STUDY_ADDRESS, STUDY_STATUS, STUDY_COST, STUDY_COMMUMITY, STUDY_LOCATION
        ) VALUES (
            SEQ_BOARD_NO.CURRVAL,
            TRUNC(DBMS_RANDOM.VALUE(2, 6)), -- STUDY_NM을 2에서 5 사이의 무작위 값으로 설정
            ...
            'KAKAOOPENTALK', -- STUDY_COMMUNITY를 'KAKAOOPENTALK'로 설정
            CASE -- STUDY_LOCATION을 무작위로 설정
                WHEN DBMS_RANDOM.VALUE(1, 8) < 2 THEN '서울'
                WHEN DBMS_RANDOM.VALUE(1, 8) < 3 THEN '부산'
                WHEN DBMS_RANDOM.VALUE(1, 8) < 4 THEN '인천'
                WHEN DBMS_RANDOM.VALUE(1, 8) < 5 THEN '광주'
                WHEN DBMS_RANDOM.VALUE(1, 8) < 6 THEN '울산'
                WHEN DBMS_RANDOM.VALUE(1, 8) < 7 THEN '대전'
                WHEN DBMS_RANDOM.VALUE(1, 8) < 8 THEN '대구'
                ELSE '경기'
            END
        );
    END LOOP;
...
-- 정산하기 테이블 데이터 삽입
INSERT INTO SETTLE (MEMBER_NO, MEMBER_NO2, CLASS_NO, SETTLE_STATUS, COMMISSION, SETTLE_DATE)
SELECT 
    C.MEMBER_NO,        -- TUTOR의 MEMBER_NO
    P.MEMBER_NO,        -- PAID 테이블의 수강생 MEMBER_NO
    C.CLASS_NO,         -- CLASS 테이블의 CLASS_NO
    '1',                -- SETTLE_STATUS (정산 가능)
    CASE 
        WHEN DBMS_RANDOM.VALUE(1, 4) < 2 THEN 5
        WHEN DBMS_RANDOM.VALUE(1, 4) < 3 THEN 10
        WHEN DBMS_RANDOM.VALUE(1, 4) < 4 THEN 15
        ELSE 20
    END,                -- COMMISSION을 5, 10, 15, 20 중 랜덤하게 설정
    SYSDATE             -- SETTLE_DATE 기본값
FROM 
    PAID P
JOIN 
    CLASS C ON P.CLASS_NO = C.CLASS_NO
JOIN 
    TUTOR T ON C.MEMBER_NO = T.MEMBER_NO
ORDER BY 
    C.MEMBER_NO, P.MEMBER_NO, C.CLASS_NO;
...
UPDATE MEMBER SET MEMBER_EMAIL ='register@naver.com',
MEMBER_NM ='튜터등록', MEMBER_PW='pass01'
WHERE MEMBER_NO = 99;
...
DECLARE
    random_date DATE;
BEGIN
    -- 현재 시각에서 7일 전의 시각을 구합니다
    random_date := SYSDATE - TRUNC(DBMS_RANDOM.VALUE(1, 7));
    
    -- MEMBER_NO가 501에서 506까지 랜덤하게 선택하여 데이터를 삽입합니다
    FOR i IN 501..506 LOOP
        INSERT INTO "PAID" ("MEMBER_NO", "CLASS_NO", "PAYMENT", "PAYMENT_STATUS", "PAYMENT_DATE")
        VALUES (i, 100, '카드결제', 'Y', random_date);
    END LOOP;
END;
/
...
BEGIN
    ...
    FOR i IN 507..513 LOOP
        INSERT INTO "PAID" ("MEMBER_NO", "CLASS_NO", "PAYMENT", "PAYMENT_STATUS", "PAYMENT_DATE")
        VALUES (i, 101, '카드결제', 'Y', random_date);
    END LOOP;
END;
/
...
BEGIN
    ...
    FOR i IN 514..521 LOOP
        INSERT INTO "PAID" ("MEMBER_NO", "CLASS_NO", "PAYMENT", "PAYMENT_STATUS", "PAYMENT_DATE")
        VALUES (i, 102, '카드결제', 'Y', random_date);
    END LOOP;
END;
/
...
BEGIN
    ...
    FOR i IN 522..527 LOOP
        INSERT INTO "PAID" ("MEMBER_NO", "CLASS_NO", "PAYMENT", "PAYMENT_STATUS", "PAYMENT_DATE")
        VALUES (i, 103, '카드결제', 'Y', random_date);
    END LOOP;
END;
/
...
BEGIN
    ...
    FOR i IN 528..535 LOOP
        INSERT INTO "PAID" ("MEMBER_NO", "CLASS_NO", "PAYMENT", "PAYMENT_STATUS", "PAYMENT_DATE")
        VALUES (i, 104, '카드결제', 'Y', random_date);
    END LOOP;
END;
/
...
DECLARE
    randomClassNo NUMBER;   
    totalMemberNo NUMBER;
BEGIN
    SELECT COUNT(*) INTO randomClassNo FROM CLASS;
    SELECT COUNT(DISTINCT MEMBER_NO) INTO totalMemberNo FROM PAID;
    ...
    FOR j IN 1..3 LOOP  -- 외부 루프 추가 (3번 반복)
        FOR i IN 1..500 LOOP
            ...
            VALUES(SEQ_BOARD_NO.NEXTVAL,
                ...
                DEFAULT, DEFAULT, DEFAULT, DEFAULT, 
                  (SELECT MEMBER_NO FROM (
                    SELECT MEMBER_NO FROM (
                        SELECT DISTINCT MEMBER_NO FROM PAID ORDER BY DBMS_RANDOM.VALUE
                    ) WHERE ROWNUM = 1))
                , 2);
            ...
            VALUES (
                SEQ_BOARD_NO.CURRVAL,
                TRUNC(DBMS_RANDOM.VALUE(1, randomClassNo)), -- 클래스 번호 1~클래스 넘버까지 랜덤
                TRUNC(DBMS_RANDOM.VALUE(0, 6))  -- 별점 1~5까지 랜덤  
            );
            
            -- 각 리뷰에 대해 랜덤한 해시태그 1개를 삽입
            INSERT INTO HASHTAG 
            VALUES (SEQ_TAG_NO.NEXTVAL, 
                    CASE TRUNC(DBMS_RANDOM.VALUE(1, 10))
                        WHEN 1 THEN '강사님이 친절해요'
                        WHEN 2 THEN '재밌어요'
                        WHEN 3 THEN '시간가는줄 몰라요'
                        WHEN 4 THEN '가격대비 최고'
                        WHEN 5 THEN '강의 맛집'
                        WHEN 6 THEN '다음에 또 올께요'
                        WHEN 7 THEN '완강했어요'
                        WHEN 8 THEN '후회 안해요'
                        WHEN 9 THEN '고민은 시간낭비'
                    END,
                    NULL, 
                    SEQ_BOARD_NO.CURRVAL);
...
DECLARE
    ...
    dupleCheck NUMBER;
BEGIN
    SELECT COUNT(*) INTO totalReviewCount FROM REVIEW;
    SELECT COUNT(*) INTO totalMemberCount FROM MEMBER;

    FOR j IN 1..totalReviewCount LOOP  -- 총 리뷰수 만큼 반복
        FOR i IN 1..5 LOOP -- 리뷰에 들어가는 추천이므로, 5번 반복
            randomBoardNo := TRUNC(DBMS_RANDOM.VALUE(1, totalReviewCount)); -- 총 리뷰 개수에서 랜덤하게 뽑기
            randomMemberNo := TRUNC(DBMS_RANDOM.VALUE(1, totalMemberCount)); -- 총 멤버 수에서 랜덤하게 뽑기
            dupleCheck := 0;
            
            -- 만약 이미 THUMBS 테이블에 MEMBER_NO와 CLASS_NO가 같은게 들어있다면
            SELECT COUNT(*)
            INTO dupleCheck
            FROM THUMBS
            WHERE MEMBER_NO = randomMemberNo
            AND BOARD_NO = (SELECT BOARD_NO FROM (SELECT R.BOARD_NO, ROWNUM AS RN FROM REVIEW R) WHERE RN = randomBoardNo);
            
            -- 같은게 들어있지 않을 때만 INSERT 실행
            IF dupleCheck = 0 THEN
                INSERT INTO THUMBS (BOARD_NO, MEMBER_NO)
                VALUES (
                -- 랜덤 넘버를 REVIEW의 ROWNUM을 기준으로 넣어서, 보드 넘버 가져와 삽입
                (SELECT BOARD_NO FROM (SELECT R.BOARD_NO, ROWNUM AS RN FROM REVIEW R) WHERE RN = randomBoardNo), 
                randomMemberNo);
            END IF;
        END LOOP;
    END LOOP;
